...
# Authors: Yulia Rubanova and Ricky Chen Adapted: Daniel Vilariño Besteiro
###########################
...
def get_data_min_max(records):
	...
	for b, (record_id, core_name, pair_id, tt, vals, mask, labels) in enumerate(records):
...
def variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	...
	pair_ids = []
	core_names = []
	for b, (record_id, core_name, pair_id, tt, vals, mask, labels) in enumerate(batch):
		tt = tt.to(device)
		vals = vals.to(device)
		mask = mask.to(device)
	...
	#La normalización generó problemas con los archivos utilizados
