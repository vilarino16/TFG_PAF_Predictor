...
# Author: Yulia Rubanova Adapted: Daniel Vilariño Besteiro
###########################
...
parser.add_argument('--niters', type=int, default=300, help="Number of iterations")
...
parser.add_argument('--classif', action='store_true', help="Include binary classification loss -- used for Physionet dataset for hospital mortality")
...
if __name__ == '__main__':
	...
	elif args.latent_ode: #LATENT ODE
		...
		if itr % (n_iters_to_viz * num_batches) == 0:
			with torch.no_grad():
				...
				if "auc" in test_res:
					...
					if test_res["auc"] > max_auc:
						max_auc = test_res["auc"]
						logger.info("New Best Classification AUC (TEST): {:.4f}".format(test_res["auc"]))
				...
				if "Evento1" in test_res:
					logger.info("Event 1 punctuation (TEST): {:.4f}".format(test_res["Evento1"]))

				if "Evento2" in test_res:
					logger.info("Event 2 puntuation (TEST): {:.4f}".format(test_res["Evento2"]))
