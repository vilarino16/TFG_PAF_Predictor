...
class VAE_Baseline(nn.Module):
	...
	def compute_all_losses(self, batch_dict, n_traj_samples = 1, kl_coef = 1.):
		...
		device = get_device(batch_dict["data_to_predict"])
		ce_loss = torch.Tensor([0.]).to(device)
		#print(self.use_binary_classif)
		if (batch_dict["labels"] is not None) and self.use_binary_classif:
			if (batch_dict["labels"].size(-1) == 1) or (len(batch_dict["labels"].size()) == 1):
				#print("label_predictions:", info["label_predictions"].shape)
				#print("labels:", batch_dict["labels"].shape)
				logits = info["label_predictions"]

				# [n_traj_samples, batch]
				if len(logits.shape) == 2:
					logits = logits.mean(0)

				ce_loss = compute_paf_pair_loss(
					logits,
					batch_dict["labels"])
			else:
				ce_loss = compute_multiclass_CE_loss(
...
#Función de pérdida que genera los coeficientes de pérdida de la tarea de clasificación de pacientes y registros
def compute_paf_pair_loss(logits, labels):
    ...
    logits_pair = logits.reshape(-1, 2)
    labels_pair = labels.reshape(-1, 2)
    ...
    patient_logits = logits_pair.max(dim=1).values

    patient_labels = (
        labels_pair.sum(dim=1) > 0
    ).float()
    ...
    # -------------------------
    # TAREA 2
    # ¿Cuál de los dos registros?
    # -------------------------

    positive_mask = patient_labels == 1

    if positive_mask.sum() > 0:

        positive_logits = logits_pair[positive_mask]

        target_idx = labels_pair.argmax(dim=1)
        positive_targets = target_idx[positive_mask]

        loss_pair = F.cross_entropy(
            positive_logits,
            positive_targets
        )
    ...
    print("loss_patient =", loss_patient.item(),"loss_pair =", loss_pair.item())
    return loss_patient + loss_pair
