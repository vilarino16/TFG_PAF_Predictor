...
def get_next_batch(dataloader):
    ...
    # cambio a "observed_mask" por la variable binaria
    ...
    # cambio a "mask_predicted_data" por la variable binaria
...
def split_data_interp(data_dict):
    ...
    if "pair_ids" in data_dict:
        split_dict["pair_ids"] = data_dict["pair_ids"]

    if "core_names" in data_dict:
        split_dict["core_names"] = data_dict["core_names"]
    return split_dict
...
def compute_loss_all_batches(model,
    ...
    for i in range(n_batches):
        ...
        if args.classif:
            ...
            classif_predictions.append(
                results["label_predictions"]
            )
    ...
    if args.classif:
        classif_predictions = torch.cat(classif_predictions, dim=1)
        ...
        # Métricas del modelo
        if args.dataset == "PAF" or args.dataset == "PAF2":
            classif_predictions = classif_predictions.squeeze(0)
            all_test_labels = all_test_labels.squeeze(-1)
            
            try:
                labels_pair = all_test_labels.reshape(-1, 2)
                temp_preds = classif_predictions
                if temp_preds.max() > 1.0 or temp_preds.min() < 0.0:
                    temp_preds = torch.sigmoid(temp_preds)
                probs_pair = temp_preds.reshape(-1, 2)

                y_patient = (labels_pair.sum(dim=1) > 0).float()

                p_patient = probs_pair.max(dim=1).values

                total["auc"] = 0.

                if torch.sum(y_patient) > 0:
                    # Área bajo la curva
                    total["auc"] = sk.metrics.roc_auc_score(
                        y_patient.cpu().numpy(),
                        p_patient.cpu().numpy()
                    )
                    # Índice Youden para el mejor umbral
                    ...
                    best_th = 0.5
                pred_patient = (p_patient > best_th).float()

                # Evento 1: detección de pacientes en riesgo de episodios de PAF
                total["Evento1"] = sk.metrics.accuracy_score(
                    y_patient.cpu().numpy(),
                    pred_patient.cpu().numpy()
                )

                positive_mask = y_patient == 1

                true_idx = labels_pair.argmax(dim=1)
                pred_idx = probs_pair.argmax(dim=1)

                if positive_mask.sum() > 0:
                    # Evento 2: predicción del episodio de PAF
                    total["Evento2"] = sk.metrics.accuracy_score(
                        true_idx[positive_mask].cpu().numpy(),
                        pred_idx[positive_mask].cpu().numpy()
                    )
                else:
                    total["Evento2"] = 0.

                # Exactitud
                total["accuracy"] = total["Evento1"]
                total["best_threshold"] = float(best_th)

            except RuntimeError:
                print("Error: número impar de registros.")
