...
from PAF import PAF, UniquePatientBatchSampler
from PAFGlobal import PAFGlobal
...
def parse_datasets(args, device):
	...
	if dataset_name == "physionet":
		train_dataset_obj = PhysioNet('data/physionet', train=True, 
										...
										download=False, n_samples = min(10000, args.n),
										device = device)
		...
		if not args.classif:
			...
			# Only 'training' physionet samples have labels. Therefore, if we do classifiction task, we don't need physionet 'test' samples.
		...
		input_dim = vals.size(-1) #dimensionalidad
	...
	if dataset_name == "PAF":
		train_data = PAF('../data/PAF/Entrenamiento/Datos', '../data/PAFAPC_11_ReplicasAPC/Entrenamiento/REFERENCE_TRAIN.csv', n_samples=args.n, device=device)
		test_data = PAF('../data/PAF/Test/Datos','../data/PAFAPC_11_ReplicasAPC/Test/REFERENCE_TEST.csv', n_samples=100, device=device)

		# En lugar de mezclar y separar, usamos lo que ya tenemos en carpetas
		input_dim = train_data[0][2].size(-1)
		...
		# Necesitamos min/max globales para normalizar igual ambos sets
		data_min, data_max = get_data_min_max(train_data[:len(train_data)] + test_data[:len(test_data)])

		train_sampler = UniquePatientBatchSampler(train_data.patient_ids, batch_size=batch_size)
		test_sampler = UniquePatientBatchSampler(test_data.patient_ids, batch_size=batch_size)
		...
		print("N_Train_Batches:", len(train_dataloader))
		print("N_Test_Batches:", len(test_dataloader))
		attr_names = train_data.params
