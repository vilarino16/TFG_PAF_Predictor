...
def compute_loss_all_batches(model,
    ...
    if args.classif:
        ...
        if args.dataset == "PAF" or args.dataset == "PAF2":
            # Repetimos las etiquetas para que coincidan con el número de trayectorias muestreadas por la Latent ODE
            # all_test_labels: [n_samples, n_labels] -> [n_traj, n_samples, n_labels]
            all_test_labels = all_test_labels.repeat(n_traj_samples, 1, 1)
            try:
                labels_paciente_raw = all_test_labels.reshape(-1, 2)
                ...
                if classif_predictions.max() > 1.0 or classif_predictions.min() < 0.0:
                    classif_predictions = torch.sigmoid(classif_predictions)
                preds_max = 0.0
                even1_max = 0.0
                even2_max = 0.0
                thMax = 0.0
                preds_best = (classif_predictions > 0.5).float()
                for th in np.arange(0.1, 0.9, 0.05):
                    preds = (classif_predictions > th).float()
                    val = sk.metrics.accuracy_score(all_test_labels.cpu().detach().numpy(),
                                                    preds.cpu().detach().numpy())
                    # Lógica Evento1: Si cualquiera de los dos registros es 1, el paciente es 1
                    preds_paciente_raw = (temp_preds > th).float().reshape(-1, 2)
                    true_paciente = (labels_paciente_raw.sum(dim=1) > 0).float()
                    pred_paciente = (preds_paciente_raw.sum(dim=1) > 0).float()
                    val1 = sk.metrics.accuracy_score(true_paciente.cpu(), pred_paciente.cpu())
                    # Evento2
                    val2 = sk.metrics.recall_score(all_test_labels.cpu().numpy(), preds.cpu().numpy())
                    if val > preds_max:
                        preds_max = val
                        thMax = th
                        preds_best = preds
                ...
                print("Error: El número de registros no es par. No se puede calcular Evento1.")
                total["Evento1"] = 0.
            dirname = "plots/" + str(experimentID) + "/"
            os.makedirs(dirname, exist_ok=True)
            ...
            if torch.sum(all_test_labels) != 0.:
                print("Number of labeled examples: {}".format(len(all_test_labels.reshape(-1))))
                print("Number of examples with PAF 1: {}".format(torch.sum(all_test_labels == 1.)))
